...
The GitHub Actions workflow in `.github/workflows/ci.yml` builds, tests, and starts the desktop application on Windows, macOS, and Linux; the Linux startup check runs under Xvfb. It also publishes self-contained `win-x64`, `osx-arm64`, and `linux-x64` artifacts for the desktop editor.
...
A stable semantic-version tag in the form `vMAJOR.MINOR.PATCH` creates a GitHub Release when the tagged commit belongs to `main`. The release contains a Windows `.zip` and macOS/Linux `.tar.gz` archives of the self-contained desktop editor; the Unix archives preserve executable permissions.
